...
public sealed class CreateNewsSourceCommandValidator : AbstractValidator<CreateNewsSourceCommand>
{
    public CreateNewsSourceCommandValidator()
    {
        ...
        // Unattended login is forbidden for OTP / MFA / manual-required sources. The platform
        // refuses to persist a stored password for such sources and forces ManualLoginRequired=true.
        RuleFor(x => x.CredentialPassword).NotEmpty()
            .When(x => x.SourceType == NewsSourceType.WebPortalLogin && !IsManualAssisted(x));
        ...
        RuleFor(x => x.IsDownloadAllowed)
            .Equal(false)
            .When(x => x.SourceType == NewsSourceType.WebPortalLogin && IsManualAssisted(x) && !x.ManualLoginRequired)
            .WithMessage("IsDownloadAllowed cannot default to true; the operator must confirm publisher terms.");
    }

    private static bool IsManualAssisted(CreateNewsSourceCommand c) =>
